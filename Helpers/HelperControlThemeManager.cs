...
using static HLWebScraper.Net.Helpers.ThemeColour;
...
internal static class HelperControlThemeManager
{
        ...
        bool enabled)
    {
        if (IsWindows10OrGreater(build: 17763))
        {
            ...
            if (IsWindows10OrGreater(build: 18985)) attribute = DWMWA_USE_IMMERSIVE_DARK_MODE;
        ...
        Control parentControl)
    {
        ...
        UseImmersiveDarkMode(handle: parentControl.Handle, enabled: themeColour == Dark);

        HelperNonStatic helperNonstatic = new();
        if (parentControl is Form) ChangeTheme(themeColour: themeColour, cItem: parentControl);

        IEnumerable<Control> c =
            helperNonstatic.GetAllControls(control: parentControl);

        foreach (Control cItem in c) ChangeTheme(themeColour: themeColour, cItem: cItem);
    }
        ...
        Control cItem)
    {
        ...
        else if (themeColour == Light)
        {
            cItem.BackColor = SystemColors.Control;
            cItem.ForeColor = SystemColors.ControlText;
